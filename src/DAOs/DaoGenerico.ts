export class DaoGenerico<T> {
  ...
  protected dados: T[] = []; //Array para armazenar os objetos 
  ...
  encontrarPorId(id: number): T | null {
    ...
    //ficaria assim se a gente tirasse a => :
  ...
  atualizar(id: number, atualizacoes: Partial<Omit<T, "id">>): T | null {
    ...
    this.dados[index] = { ...this.dados[index], ...atualizacoes } as T; //Atualiza o objeto no índice encontrado, combinando seus valores atuais com as al++terações (updates).
