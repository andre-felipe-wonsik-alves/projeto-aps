export class DAOGenerico<T> { //Define uma classe genérica que pode lidar com qualquer tipo T
    ...
    criar(entidade: Omit<T, 'id'>): T { // O parâmetro entity é do tipo T, mas sem a propriedade id (Omit<T, 'id'> remove o campo id do tipo T).
    ...
    atualizar(id: number, atualizacoes: Partial<Omit<T, 'id'>>): T | null {     //Recebe o id do objeto a ser atualizado e as alterações a serem aplicadas (updates).
